...
#  ---------------------------k -------------------------------------
    ...
    k[0] = small
    k[p.nz-1] = small
    sol = gmres(A[1:p.nz-1, 1:p.nz-1],b[1:p.nz-1],rho[1:p.nz-1]*k[1:p.nz-1],tol=1e-6, maxiter=1, M = D)
    ...
    # Solving
    sol = gmres(A[1:p.nz-1, 1:p.nz-1],b[1:p.nz-1],cp.sqrt(rho[1:p.nz-1])*om[1:p.nz-1], tol=1e-6, maxiter=1, M = D )
...
def Momentum(u,mut,mu,p,mesh,rho):
    ...
    sol = gmres(A[1:p.nz-1,1:p.nz-1],b[1:p.nz-1],u[1:p.nz-1],tol=1e-6, maxiter=1)
    u[1:p.nz-1] = sol[0]
    # u[1:p.nz-1] = cp.linalg.solve(A[1:p.nz-1, 1:p.nz-1], b[1:p.nz-1])
